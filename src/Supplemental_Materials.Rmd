...
author: 
 - "Erich Seamon, Jennifer A. Mattera, Sarah A. Keim, Esther M. Leerkes, Jennifer L. Rennels," 
 - "Andrea J. Kayl, Kirsty M. Kulhanek, Darcia Narvaez, Sarah M. Sanborn, Jennifer B. Grandits," 
 - "Christine Dunkel Schetter, Mary Coussons-Read, Amanda R. Tarullo, Sarah J. Schoppe-Sullivan," 
 - "Moriah E. Thomason, Julie M. Braungart-Rieker, Julie C. Lumeng, Shannon N. Lenze, "
 - "Lisa M. Christian , Darby E. Saxbe, Laura R. Stroud, Christina M. Rodriguez, "
 - "Stephanie Anzman-Frasca, Maria A. Gartstein" 
date: "01/04/2024"
output:
...
colnames(Surgency) <- "Positive Affectivity"
Negative_Affect <- as.data.frame(questions[c(3,4,9,10,16,17,22,23,28,29,32,33)])
colnames(Negative_Affect) <- "Negative Affectivity"
...
kable(Surgency, caption = "Infant Behavior Questionnaire-Revised Very Short Form Items - Positive Affectivity") %>%
kable_styling(position = "left", font_size = 8) %>% row_spec(0,bold=TRUE) %>%
  column_spec(1, width = "60em")

kable(Negative_Affect, caption = "Infant Behavior Questionnaire-Revised Very Short Form Items - Negative Affectivity") %>%
kable_styling(position = "left", font_size = 8) %>% row_spec(0,bold=TRUE) %>%
  column_spec(1, width = "60em")
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.width = 11, fig.height = 7.5, out.width= 11 }

library(kableExtra)
library(gridExtra)
library(tibble)

researchers <- c("Keim", "Leerkes", "Leerkes", "Rennels", "Rennels", "Rennels", "Narvaez", "Sanborn & Bisson", "Sanborn & Bisson", "Dunkel Schetter & Coussons-Read", "Dunkel Schetter & Coussons-Read", "Tarullo", "Schoppe-Sullivan", "Thomason", "Thomason", "Braungart-Rieker", "Lumeng", "Lenze", "Lenze", "Christian", "Christian", "Christian", "Christian", "Saxbe", "Saxbe",  "Stroud", "Stroud", "Rodriguez", "Anzman-Frasca")
samplegrouping <- c("Sample 1", "Sample 1", "Sample 1", "Sample 1", "Sample 1", "Sample 2","Sample 1","Sample 1","Sample 1","Sample 1","Sample 1","Sample 1","Sample 1","Sample 1","Sample 1","Sample 1","Sample 1","Sample 1","Sample 1","Sample 1","Sample 1","Sample 2","Sample 2","Sample 1","Sample 1","Sample 1","Sample 1","Sample 1","Sample 1")
samplesize <- c("121", "Preterm:8", "Full-term:113", "Preterm:23", "Full-term:32", "Full-term:7","48", "Preterm:18", "Full-term:35", "Preterm:8", "Full-term:35", "31", "30", "Preterm:12",  "Full-term:8", "19", "15", "Preterm:4", "Full-term:9", "Preterm:2", "Full-term:4", "Preterm:2", "Full-term:7", "Preterm:1", "Full-term:10", "Preterm:2", "Full-term:4", "5", "2")
infantage <- c("48.00-72.00", "24.43-62.71","51.71-71.71","27.57-46.14","25.00-56.57","26.00-28.14","48.00-67.00","10.29-60.71","6.14-63.14","25.14-57.00","51.29-63.71","44.20-56.20","34.00-51.00","24.07-33.27","28.27-30.38","21.19-32.09","17.14-24.00","27.00-30.00","52.00-69.00","8.00","8.00","12.00-16.00","12.00-24.00","28.51","13.52-33.09","9.61-11.97","3.44-4.03","24.00","10.57-16.43")
adjustedinfantage <- c("40.00-64.00","21.00-58.00","","22.43-42.00","","","","4.29-56.71","","20.14-53.00","","","","13.39-30.13","","","","23.00-26.00","","4.00-4.29","","8.43-12.57","","25.36","","3.32-6.69","","","")
gendermale <- c("52.1","50.0","46.9","47.8","56.3","42.9","56.3","61.1","53.8","62.5","53.1","61.3","50.0","75.0","25.0","57.9","46.7","75.0","55.6","50.0","50.0","50.0","25.0","0.00","70.0","50.0","50.0","60.0","100.0")
race <- c("32.2","62.5","51.3","56.5","40.6","42.9","25.0","Not Collected", "Not Collected","87.5","51.4","22.6", "6.6","83.3","75.0","26.3","26.7","100.0","77.8","50.0","50.0","0.00","50.0","0.00","20.0","100.0","50.0","40.0","50.0")
description <- c("Sample of preterm infants recruited in NICU","Community sample of infants born to primiparous mothers","Community sample of infants born to primiparous mothers","Community sample of infants","Community sample of infants","Community sample of full-term infants","Community sample of full-term infants","Community sample of infants","Community sample of infants","Community sample of infants","Community sample of infants","Community sample of full-term, healthy infants","Community sample of infants born to primiparous mothers","Infants born to high-risk mothers from low socioeconomic backgrounds","Infants born to high-risk mothers from low socioeconomic backgrounds","Community sample of full-term infants without any developmental delays","Community sample of full-term, healthy infants","Infants born to mothers with prenatal depression","Infants born to mothers with prenatal depression","Sample of infants born to mothers with low-risk pregnancy (e.g., no major health problems, singleton pregnancy)","Sample of infants born to mothers with low-risk pregnancy (e.g., no major health problems, singleton pregnancy)","Sample of infants born to mothers with low-risk pregnancy (e.g., no major health problems, singleton pregnancy)","Sample of infants born to mothers with low-risk pregnancy (e.g., no major health problems, singleton pregnancy)","Community sample of infants born to primiparous mothers","Community sample of infants born to primiparous mothers","Community sample of infants oversampled for mothers who smoked cigarettes during pregnancy","Community sample of infants oversampled for mothers who smoked cigarettes during pregnancy","Full-term infants born to primiparous mothers with elevated sociodemographic risk","Community sample of full-term infants born to primiparous mothers")

df <- tibble(researchers, samplegrouping, samplesize, infantage, adjustedinfantage, gendermale, race, description)
colnames(df) <- c("Researcher(s)","Sample Group",  "Sample Size (n)", "Chronological Infant Age (weeks)", "Adjusted Infant Age (weeks)", "Gender (% male)", "Race (% non-white)", "Sample Description")

  kbl(df, caption = "Sample descriptions. Sample sizes were determined by our ability to match infants born preterm and comparison babies based on infant age (chronological and adjusted for prematurity).") %>% 
  collapse_rows(columns = c(1), valign = "middle",
                latex_hline = "full") %>% kable_styling(position = "left", bootstrap_options = c("condensed"), font_size = 7) %>%
  column_spec(8, width = "22em") %>%
  column_spec(1, width = "15em") %>%
  column_spec(c(2), width = "6em") %>% 
  column_spec(c(3), width = "7em") %>%
  column_spec(c(4), width = "8em") %>% 
  column_spec(c(5), width = "6em") %>% 
  column_spec(c(6:7), width = "5em") %>% row_spec(0, bold = TRUE)
...
items <- c("Surgecy", "Item 1","Item 2","Item 7","Item 8","Item 13","Item 14","Item 15","Item 20","Item 21","Item 26","Item 27","Item 36","Item 37", "Negative Affect", "Item 3","Item 4","Item 9","Item 10","Item 16","Item 17","Item 22","Item 23","Item 28","Item 29","Item 32","Item 33", "Effortful Control",  "Item 5","Item 6","Item 11","Item 12","Item 18","Item 19","Item 24","Item 25","Item 30","Item 31","Item 34","Item 35")

mean_preterm <- c(5.25,4.84,6.23,5.69,4.93,5.58,6.03,5.45,5.65,5.82,4.36,3.81,5.66,4.14,4.31,4.93,4.48,4.29,4.19,4.25,3.53,3.10,4.07,4.07,4.72,5.28,4.65,5.29,5.46,4.95,4.05,5.62,5.96,4.31,6.07,5.19,5.78,3.54,6.35,5.71)

sd_preterm <- c(0.93,1.69,1.00,1.49,2.01,1.40,1.37,1.75,1.44,1.31,2.04,2.32,1.50,1.84,1.13,1.58,2.05,1.86,1.94,1.84,1.91,1.66,1.73,2.22,1.80,1.54,2.10,0.72,1.50,1.71,1.97,1.12,1.19,1.86,1.10,1.34,1.34,1.94,0.91,1.23)


range_preterm <- c("2.17-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","2.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","2.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","3.42-7.00","1.00-7.00","1.00-7.00","1.00-7.00","2.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","2.00-7.00","1.00-7.00")

mean_chron <- c(5.29,4.98,6.25,5.76,5.19,5.43,6.03,5.43,5.54,5.84,4.49,4.03,5.40,4.06,4.08,4.57,4.20,4.33,4.13,4.23,3.29,2.85,3.86,3.81,4.32,5.05,4.31,5.24,5.51,4.96,3.71,5.54,5.86,4.18,6.14,5.32,5.59,3.28,6.17,5.55)

sd_chron <- c(0.83,1.59,1.15,1.44,1.93,1.51,1.36,1.56,1.44,1.21,1.91,2.06,1.54,1.73,1.02,1.60,1.91,1.85,1.92,1.73,1.77,1.55,1.66,1.96,1.58,1.50,1.88,0.77,1.37,1.62,2.02,1.25,1.17,1.77,1.00,1.17,1.48,1.89,1.02,1.29)

range_chron <- c("2.00-6.73","1.00-7.00","2.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.75-6.33","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","3.08-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","3.00-7.00","2.00-7.00","2.00-7.00","1.00-7.00","2.00-7.00","1.00-7.00")

mean_adjust <- c(5.07,4.79,6.09,5.65,4.74,5.62,5.83,5.02,5.81,5.72,4.03,3.27,5.30,4.06,3.89,4.49,3.76,3.98,4.10,3.88,3.18,2.74,3.87,3.45,4.29,4.79,4.14,5.24,5.50,4.79,3.71,5.50,5.80,4.16,6.18,5.26,5.77,3.30,6.35,5.46)

sd_adjust <- c(0.84,1.65,1.04,1.35,2.11,1.20,1.39,1.64,1.32,1.21,1.97,2.05,1.51,1.70,0.94,1.50,1.84,1.72,1.83,1.68,1.53,1.37,1.57,2.02,1.62,1.44,2.05,0.70,1.42,1.61,2.05,1.08,1.26,1.85,0.91,1.18,1.30,1.83,0.83,1.29)

range_adjust <- c("1.67-6.92","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","2.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.45-6.42","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","2.00-7.00","1.00-7.00","2.92-6.90","1.00-7.00","1.00-7.00","1.00-7.00","2.00-7.00","1.00-7.00","1.00-7.00","3.00-7.00","1.00-7.00","1.00-7.00","1.00-7.00","2.00-7.00","1.00-7.00")
...
df <- tibble(items, mean_preterm, sd_preterm, range_preterm, mean_chron, sd_chron, range_chron, mean_adjust, sd_adjust, range_adjust)
colnames(df) <- c("Factors/Items", "Mean", "SD", "Range", "Mean", "SD", "Range", "Mean", "SD", "Range")

  kbl(df, caption = "Descriptive statistics for the temperament factors and items by term status.") %>%
  add_header_above(c(" ", "Preterm" = 3, "Chronological" = 3, "Age Adjusted"= 3)) %>% kable_styling(position = "left", bootstrap_options = c("condensed"), font_size = 7) %>%
  column_spec(c(2:10), width = "4em") %>%
  column_spec(c(1), width = "6em") %>% row_spec(0, bold = TRUE)
```
...
```{r table2, echo = FALSE, message=FALSE, fig.env='table', fig.align='center', fig.cap=paste("Sample Descriptions."), out.width='1\\linewidth', fig.topcaption=TRUE, fig.pos='H'}
...
p1$name <- c("Effortful Control","Negative Affectivity", "Positive Affectivity")
# p1name <- gsub("^.*?_","",p1$name)
# p1name <- gsub("^.*?_","",p1name)
p1$category <- p1$name
# 
p2$name <- c("Effortful Control","Negative Affectivity", "Positive Affectivity")
...
p1a <- ggplot(p1[1:3,], aes(x=Overall, y=name, fill=category)) + geom_bar(aes(y=name), stat="identity") + theme_classic(base_size = 24) + xlab("Importance %") + ylab("Category") + xlim(0,.8)
p2a <- ggplot(p2[1:3,], aes(x=Overall, y=name, fill=category)) + geom_bar(aes(y=name), stat="identity") + theme_classic(base_size = 24) + xlab("Importance %") + ylab("Category") + xlim(0,.8)
...
levels(p1$category) <- c("Effortful Control","Negative Affectivity", "Positive Affectivity" )

p2name <- gsub("^.*?_","",p2$name)
p2name <- gsub("^.*?_","",p2name)
p2$category <- as.factor(p2name)
levels(p2$category) <- c("Effortful Control","Negative Affectivity", "Positive Affectivity" )
...
p1a <- ggplot(p1[1:20,], aes(x=Overall, y=name, fill=category)) + geom_bar(aes(y=name), stat="identity")  + theme_classic(base_size = 24)+ xlab("Importance %") + ylab("Questions") + xlim(0,.05)
p2a <- ggplot(p2[1:20,], aes(x=Overall, y=name, fill=category)) + geom_bar(aes(y=name), stat="identity")  + theme_classic(base_size = 24)+ xlab("Importance %") + ylab("Questions") + xlim(0,.05)
...
R2plot_all <- ggplot(R2_long[c(1:11, 23:33),],            
               aes(x = factor(modelname, levels = c("lda","glm","svm","knn","nb","cart","c5.0","bagging","rf","gbm","adabag")),
